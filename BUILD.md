...
Everything here is what the vpk build compiles, nothing else.
...
> This is the client only — everything here compiles into the vpk, nothing
> else. The app does nothing without `middleware.py` running on your Ubuntu
> server; keep the copy you already have.
>
> **Search needs the `/search` endpoint**, which was added to `middleware.py`
> separately. Against a copy without it, search returns nothing while
