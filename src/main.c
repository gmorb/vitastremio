...
enum { SCR_CATALOG, SCR_STREAMS, SCR_PLAYING };
...
enum { WORK_NONE, WORK_CATALOG, WORK_STREAMS, WORK_POSTER, WORK_SYNC,
       WORK_SUBTRACKS, WORK_SUBS, WORK_AUDTRACKS };
...
static void request_catalog(const char *type)
{
    char path[128];
    g_search_term[0] = 0;              /* leaving search mode */
    snprintf(path, sizeof(path), "/catalog?type=%s", type);
...
{
    char path[256];
    snprintf(path, sizeof(path), "/subtracks?type=movie&id=%s",
             g_items[g_sel].id);
...
static void request_search(const char *term)
{
    ...
    if (!term[0]) { request_catalog("movie"); return; }

    snprintf(g_search_term, sizeof(g_search_term), "%s", term);
    urlesc(term, esc, sizeof(esc));
    snprintf(path, sizeof(path), "/search?type=movie&q=%s", esc);
...
{
    char path[640];
    snprintf(path, sizeof(path), "/streams?type=movie&id=%s", it->id);
...
static void handle_catalog(unsigned int pressed)
{
    ...
    if (g_tapped) {
        ...
        for (int i = g_scroll; i < g_item_count && i < g_scroll + page; i++) {
            ...
            if (i == g_sel) request_streams(&g_items[i]);
            else            g_sel = i;
            return;
        }
    }
    ...
    if ((pressed & SCE_CTRL_CROSS) && g_item_count > 0)
        request_streams(&g_items[g_sel]);   /* screen flips on completion */
    ...
    if (pressed & SCE_CTRL_TRIANGLE) request_catalog("movie");  /* clears search */
}
